...
- **Success Metrics:**
  ...
  - **Capture latency:** creating a task, logging a workout, or logging an expense takes <10 seconds from "open app" to "saved," with zero required setup step before first use.
...
**In-Scope (v1 MVP):**
- Tasks (due dates, status, project/area linking)
...
- Global Quick Capture (one shortcut/button to add a task, expense, or workout from anywhere in the app without navigating)
...
- *Step-by-step flow:*
  1. Lands on the **Today** dashboard: today's Routine checklist, tasks due today, and a running weekly Finance snapshot.
  2. Checks off Routine items one by one; each check is optimistic (updates instantly, syncs in background).
  3. Adds a new task via Quick Capture mid-day without leaving whatever screen they're on.
  ...
  5. Returns to Today dashboard at day's end; sees Routine completion % and tasks closed.
...
| **FR-03** | Task CRUD | Create, edit, complete, delete, and due-date a task; optional link to Project/Goal/Routine. | P0 | State (including completion) persists across app reloads with no re-fetch flash of stale data. |
...
| **FR-07** | Global Quick Capture | A persistent, keyboard-accessible entry point to create a task or Fitness log entry from any screen. | P0 | Given the user is on any screen, when they trigger Quick Capture, then they can create and save a new entry in ≤3 interactions without a full page navigation. |
...
| **FR-09** | Search & Filter | Full-text search across tasks, Fitness logs, and Routines; debounced query. | P1 | Given a search term, when the user types, then results update within 150ms of the debounce window closing, across all four domains. |
| **FR-10** | Today Dashboard | Aggregated view: today's Routine checklist and tasks due today. | P0 | Given data exists across the three domains, when the user loads the app, then the Today dashboard renders both summaries in a single view with no per-domain navigation required. |
...
- Key UI components: Routine checklist widget, "Due Today" task list, Quick Capture button (persistent, floating or header-anchored), global search bar.
...
**Screen 2: Domain List View (Tasks / Fitness / Routines — shared layout pattern)**
...
- Key UI components: domain-specific field editor (Task fields vs. Fitness fields vs. Finance fields), Link Engine picker (search-and-attach any other entity), activity/link feed showing everything this entity is connected to, delete/archive action.
...
- Key UI components: domain selector (Task / Fitness), minimal field set per domain (only what's required to save), save-and-close vs. save-and-add-another.
...
- **Performance:** First Contentful Paint (FCP) < 1.2s on the Today dashboard; all writes (task complete, Routine check, quick-add) are optimistic — UI updates before the network round-trip completes.
...
- Finance is deferred out of the MVP entirely, not built alongside Tasks/Fitness/Routines; expense/income/balance/assets/debts/credit tracking all move to a later version, with the schema extension path above kept for when that happens.

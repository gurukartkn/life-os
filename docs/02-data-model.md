...
> **v2 Goals, Stage 5a (2026-09-26):** `links` accepts `workout` and `exercise` as source and target types (`supabase/migrations/20260926150000_links_workout_exercise.sql`). Then the goals expand step: `goals` gains `achieved_on` (the local date, in the user's timezone, a goal was achieved; set only while `status = 'achieved'`), `status` defaults to `active`, and the v2 values `achieved` / `dropped` replace `completed` / `abandoned` (existing rows moved; the old values stay allowed until the contract migration). A goal is only ever a link's source (`links_goal_is_source_check`: `target_type <> 'goal'`), and `links_target_idx` on `(target_type, target_id)` serves the clean-up when a task or workout is deleted. Applied to life-os-dev by `supabase/migrations/20260926170000_goals_expand.sql`; the inverse is `supabase/rollbacks/20260926170000_goals_expand.down.sql`. **Neither is applied to life-os-prod yet**, and a prod link with `target_type = 'goal'` (v1 "link a workout log to a goal") would make the expand step fail there, so prod needs checking first. The contract migration (narrowing `status`, `not null`, an `achieved_on` check) is not written yet.
